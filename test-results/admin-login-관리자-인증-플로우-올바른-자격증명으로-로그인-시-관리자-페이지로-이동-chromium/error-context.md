...
- Name: admin-login.spec.ts >> 관리자 인증 플로우 >> 올바른 자격증명으로 로그인 시 관리자 페이지로 이동
- Location: e2e/admin-login.spec.ts:14:9
...
```ts
  ...
  13 | {
  14 |     await page.goto('http://admin.localhost:3000/admin/login');
  15 |     
...
> 17 |     await page.fill('input[type="email"]', 'admin@shop.com');
  ...
  27 |     await page.waitForURL('http://admin.localhost:3000/admin');
  28 |     
