...
### 3.6. Git Commit Rule (한글 커밋 강제 규칙) - [절대 준수]
- **한글 커밋 강제**: 에이전트가 코드를 커밋하거나 형상 관리 명령을 수행할 때, 커밋 메시지는 예외 없이 **반드시 한국어(Korean)**로만 작성해야 합니다. 영어나 기타 외국어로 커밋 메시지를 생성하는 것은 엄격히 금지됩니다.
  - 예시: `git commit -m "feat: 500GB+ 로컬 스토리지 분리 적재 및 Cloudflare CDN 프록시 캐싱 연동"`
